...
def main():
    ...
    parser.add_argument(
        "--text",
        help="Texto único para testar"
    )

    parser.add_argument(
        "--input",
        help="Arquivo Markdown para processar (alternativa a --text)"
    )
    ...
    if not os.path.exists(args.speaker_wav):
        print(f"❌ Erro: Speaker WAV não encontrado: {args.speaker_wav}", file=sys.stderr)
        sys.exit(1)

    if args.input and not os.path.exists(args.input):
        print(f"❌ Erro: Arquivo Markdown não encontrado: {args.input}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"\n🎉 Teste concluído!")
        print(f"   Compare com: audiobook.mp3 (Kokoro)")
